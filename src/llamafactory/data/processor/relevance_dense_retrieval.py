...
@dataclass
class RelevanceDenseRetrievalDatasetProcessor(DatasetProcessor):
    def preprocess_dataset(self, examples: dict[str, list[Any]]) -> dict[str, list[Any]]:
        ...
        for idx in range(len(examples["_src"])):
            ...
            for pair_idx in range(num_pairs): # num_pairs其实为1
                ...
                tgt, dr_src, dr_tgt, dr_src_r,dr_tgt_r,is_dr,label_rel = tgt.split('\x01')
                ...
                # 相关性 dr sft src and tgt
                dr_src_r_msg = self.template.format_user.apply(content=dr_src_r)
                dr_tgt_t_msg = self.template.format_user.apply(content=dr_tgt_r)
                dr_r_source_ids = self.tokenizer.encode(dr_src_r_msg[0], add_special_tokens=False)
                dr_t_target_ids = self.tokenizer.encode(dr_tgt_t_msg[0], add_special_tokens=False)

                # 截断到最大长度
                dr_r_source_len, _ = infer_seqlen(len(dr_r_source_ids), 0, self.data_args.cutoff_len)
                dr_t_target_len, _ = infer_seqlen(len(dr_t_target_ids), 0, self.data_args.cutoff_len)
                dr_r_source_ids = dr_r_source_ids[:dr_r_source_len]
                dr_t_target_ids = dr_t_target_ids[:dr_t_target_len]
                ...
                dr_r_source_label_ids = [IGNORE_INDEX] * len(dr_r_source_ids)
                dr_t_tgt_label_ids = [IGNORE_INDEX] * len(dr_t_target_ids)
                ...
                if self.template.efficient_eos:
                    ...
                    dr_r_source_ids.append(self.tokenizer.eos_token_id)
                    dr_t_target_ids.append(self.tokenizer.eos_token_id)
                    dr_r_source_label_ids.append(self.tokenizer.eos_token_id)
                    dr_t_tgt_label_ids.append(self.tokenizer.eos_token_id)

                model_inputs['is_dr'].append(float(is_dr))

                model_inputs["input_ids"].append(input_ids + dr_source_ids + dr_target_ids+dr_r_source_ids+dr_t_target_ids)
                model_inputs["attention_mask"].append([1] * len(input_ids) + [2] * len(dr_source_ids) + [3] * len(dr_target_ids)+[4]*len(dr_r_source_ids)+[5]*len(dr_t_target_ids))
                model_inputs["labels"].append(label_ids + dr_source_label_ids + dr_tgt_label_ids+dr_r_source_label_ids+dr_t_tgt_label_ids)
                model_inputs['dr_slice'].append([len(input_ids) + len(dr_source_ids), len(input_ids) + len(dr_source_ids) + len(dr_target_ids),len(input_ids) + len(dr_source_ids) + len(dr_target_ids)+len(dr_r_source_ids),len(input_ids) + len(dr_source_ids) + len(dr_target_ids)+len(dr_r_source_ids)+len(dr_t_target_ids)])
                model_inputs['label_rel'].append(float(label_rel))
        return model_inputs
