...
@dataclass
class PNLDenseRetrievalDatasetProcessor(DatasetProcessor):
    def preprocess_dataset(self, examples: dict[str, list[Any]]) -> dict[str, list[Any]]:
        ...
        for idx in range(len(examples["_src"])):
            ...
            for pair_idx in range(num_pairs): # num_pairs其实为1
                ...
                model_inputs["input_ids"].append(input_ids + dr_source_ids + dr_target_ids)
                model_inputs["attention_mask"].append([1] * len(input_ids) + [2] * len(dr_source_ids) + [3] * len(dr_target_ids))
                model_inputs["labels"].append(label_ids + dr_source_label_ids + dr_tgt_label_ids)
                model_inputs['dr_slice'].append([len(input_ids) + len(dr_source_ids), len(input_ids) + len(dr_source_ids) + len(dr_target_ids)])
