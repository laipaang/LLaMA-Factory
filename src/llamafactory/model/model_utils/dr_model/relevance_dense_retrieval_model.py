...
class Qwen2ForCausalLMRelevanceDenseRetrieval(Qwen2PreTrainedModel, GenerationMixin):
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if self.use_dense_retrieval:
            if dr_slice is not None:
                ...
                batch_indices = torch.arange(batch_size).unsqueeze(-1).expand(-1, 4)  # shape [batch_size, 4]
                dr_logits = hidden_states[batch_indices, dr_slice - 1, :]
                dr_logits = self.context_feature_model(dr_logits)
                label_rel = label_rel.view(-1)
                is_dr = is_dr.view(-1)
                ...
                # 相关性q、ad emb
                query_r_embedding = dr_logits[:, 2, :].squeeze(dim=1).contiguous()
                ad_r_embedding = dr_logits[:, 3, :].squeeze(dim=1).contiguous()
                ...
                # 添加相关性loss
                # 计算相似度
                sim = F.cosine_similarity(query_r_embedding, ad_r_embedding)
                # sim_norm = (sim + 1) / 2
                # 过sigmoid
                # bce_loss = nn.BCEWithLogitsLoss(reduction='none')
                # sim_label = label_rel.float()
                # sim_loss = bce_loss(sim, sim_label)
                # mse
                mse_loss = nn.MSELoss(reduction='none')
                # sim_label = label_rel.float()
                sim_loss = mse_loss(sim, label_rel)
                    
                sim_loss = (sim_loss*is_dr).mean() * self.dr_weight

                loss = loss + dr_celoss + sim_loss
...
if __name__ =='__main__':
    ...
    model = Qwen2ForCausalLMRelevanceDenseRetrieval.from_pretrained(model_path)
